...
if ((isNull _target) && {_unit getVariable [QGVAR(isEscorting), false]}) then {
    ...
    {
        if ((_x getVariable [QGVAR(isHandcuffed), false]) || (_x getVariable [QGVAR(isHandcuffed), false])) exitWith {
            _target = _x;
        };
    } forEach (attachedObjects _unit);
};
if ((isNull _target) || {(vehicle _target) != _target} || {!(_target getVariable [QGVAR(isZiptied), false])} || {!(_target getVariable [QGVAR(isHandcuffed), false])}) exitWith {WARNING("");};
